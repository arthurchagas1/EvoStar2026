# -*- coding: utf-8 -*-
"""
Evo-Reasoner++ (Coach-enabled, No-Gold) — Evolution WITHOUT MAP-Elites (QD ablation)
Diagnosis patch + GEJ (Global End-of-Generation Judge), consensus-first & richer logs.

This variant DISABLES the QD archive and per-cell selection:
- USE_PERSISTENT_MAP_ELITES = False
- USE_MAP_ELITES_FOR_SELECTION = False
All other components remain identical (AUTHOR/JUDGE, Elo tournaments, mutations/crossover, GEJ panel, coach, strict schema/repair).
"""
...
# Evolution settings
P, GENERATIONS, MATCHES_PER_GEN = 12, 12, 16
...
# Paths / IO
OUT_DIR, GLOBAL_SUMMARY = "evo_runs_noqd_12_gen", "summary.json"
...
# Ablations / knobs
USE_PERSISTENT_MAP_ELITES = False          # <== QD OFF
USE_MAP_ELITES_FOR_SELECTION = False       # <== QD OFF
USE_CROSSOVER = True
    ...
    with torch.inference_mode():
        ...
        while i<len(prompts):
            ...
            tries = [
                ...
                ("gpu",1,False,12 if max_new<12 else max(12,max_new//2)),
...
class MapElites:
    """Kept for compatibility; not used when QD is OFF."""
    def __init__(self):
...
def panel_judge_select_token(task:str, inds: List[Individual], jt, jm, seed:int, messages:List[Dict[str,Any]], fused_ind:Optional[Individual]=None)->Tuple[Optional[str], Optional[str], Dict[str,Tuple[int,int]]]:
    ...
    outs, _ = robust_generate_batch(jt, jm, [pr], MAX_NEW_TOKENS_PANEL_JUDGE, None, None, seed, 1, True)
    raw = outs[0] if outs else ""
...
def run_evolution_for_task(task_id:str, task_text:str, at, am, jt, jm, messages:List[Dict[str,Any]])->Dict[str,Any]:
    ...
    for gen in range(1,GENERATIONS):
        ...
        else: archive.archive = {archive.key(x):x for x in pop}  # harmless when QD is off
        swap_active_models(am, jm, author_on_gpu=False)
    ...
    swap_active_models(am, jm, author_on_gpu=True)
    champs=sorted(pop, key=lambda x:x.elo, reverse=True)[:max(3, ELITISM_KEEP)]  # no-QD: use global best
...
def main():
    ...
    run_tag = "noqd" if (not USE_PERSISTENT_MAP_ELITES and not USE_MAP_ELITES_FOR_SELECTION) else "qd"
    run_dir = os.path.join(OUT_DIR, f"evo_{AUTHOR_MODEL_NAME.split('/')[-1]}_{loader_mode}_{run_tag}")
