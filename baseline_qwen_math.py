...
MODEL_NAME   = "Qwen/Qwen2.5-14B-Instruct"  # bigger than 7B, instruction-tuned
BENCH_PATH   = "/scratch/pedro.bento/evostar/data/math_benchmark.jsonl"
OUT_DIR      = "qwen_simple_runs_14b"
...
def load_model(name: str):
    ...
    def _try_all(local_path_inner: str):
        ...
        for ctor in (_mdl_full_gpu, _mdl_auto_offload):
            try:
                mdl, mode = ctor(local_path_inner, DTYPE); return mdl, mode
            except Exception as e:
                last_err = e
                if torch.cuda.is_available(): torch.cuda.empty_cache()
        for ctor in (_mdl_8bit, _mdl_4bit):
            try:
                mdl, mode = ctor(local_path_inner); return mdl, mode
            except Exception as e:
                last_err = e
                if torch.cuda.is_available(): torch.cuda.empty_cache()
        raise last_err
...
def _boxed_all(s: str) -> list: return re.findall(r"\\boxed\{([^}]*)\}", s)
    ...
    for tid, q, gold_raw in pending:
        ...
        def _num_from_text(s: Optional[str]) -> Optional[float]:
            ...
            if mf:
                ...
                try: return float(_norm_num(a)) / float(_norm_num(b))
                except: pass
