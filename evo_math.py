...
# Paths / IO
OUT_DIR, GLOBAL_SUMMARY = "evo_runs", "summary.json"
BENCH_PATH, RESUME_RUN = "/scratch/pedro.bento/evostar/data/math_benchmark.jsonl", True
...
def _ctx_len(model, tok=None) -> int:
    ...
    if tok is not None:
        ...
        # HF uses a giant sentinel for "infinite" sometimes; clip it
            ...
            for t in tries:
                ...
                try:
                    if where=="gpu":
                        ...
                        for sub in chunks(micro, this_bs):
                            g, s = run_one(sub, torch.device("cpu"), False, cap); acc+=g; sec+=s
