# -*- coding: utf-8 -*-
"""
DeepSeek simple baseline — crash-safe, OOM-safe, clear FINAL_ANSWER line (no CoT prompt)
Usage: python baseline_oomsafe_progress.py

Notes:
- Exact Match (EM) against the full gold text was removed, since this baseline only cares about the final numeric answer.
- Metrics now focus on numeric correctness only (num_correct / gold_num_seen), plus throughput stats.
"""
...
OUT_DIR      = "deepseek_simple_runs_14b"

MAX_NEW_TOKENS = 384    # higher cap; early stop keeps it fast
BATCH_SIZE     = 8
REPORT_EVERY   = 10
RESUME_RUN     = True

# NEW: limit how many tasks to run this session (set to None for all)
TASK_LIMIT     = 100
...
def load_model(name: str):
    ...
    last_err = None
    for loader in (_try_full_gpu, _try_auto_offload):
        try:
            mdl, mode = loader(name, DTYPE); break
        except Exception as e:
            last_err = e
            if torch.cuda.is_available(): torch.cuda.empty_cache()
    else:
        for loader in (_try_8bit, _try_4bit):
            try:
                mdl, mode = loader(name); break
            except Exception as e:
                last_err = e
                if torch.cuda.is_available(): torch.cuda.empty_cache()
        else:
...
class FinalAnswerStopper(StoppingCriteria):
    ...
    def __init__(self, tokenizer, start_len: int, eos_id: Optional[int], lookback_tokens: int = 256):
        super().__init__(); self.tok = tokenizer; self.start_len = start_len; self.eos_id = eos_id; self.n = lookback_tokens
...
def stream_generate(tokenizer, model, questions: List[str], max_new_tokens: int, batch_size: int):
    ...
    dev = _primary_device(model); micro_bs = max(1, min(batch_size, len(questions))); max_new = max_new_tokens
    i = 0
    with torch.inference_mode():
        while i < len(questions):
            ...
            try:
                ...
                gen = model.generate(
                    **inputs, max_new_tokens=max_new, do_sample=False, temperature=1.0, top_p=1.0,
...
def main():
    ...
    run_dir = os.path.join(OUT_DIR, "deepseek-llm-7b-chat_" + mode)
    os.makedirs(run_dir, exist_ok=True)

    done = {fn[len("task_"):-5] for fn in os.listdir(run_dir) if fn.startswith("task_") and fn.endswith(".json")} if RESUME_RUN else set()
    ...
    for tid, q, gold_raw in pending:
        ...
        gold_tok_raw = (hash4_pat.search(gold_raw).group(1).strip() if (gold_raw and hash4_pat.search(gold_raw)) else _extract_token(gold_raw or ""))
